...
func (s *sectionParser) Open(_ gast.Node, reader text.Reader, _ parser.Context) (gast.Node, parser.State) {
	...
	if result := sectionStartRegex.Find(line); result != nil {
		reader.AdvanceLine()
...
func (s *sectionParser) Continue(node gast.Node, reader text.Reader, _ parser.Context) parser.State {
	...
	if result := sectionEndRegex.Find(line); result != nil {
...
// SectionMarkdownRenderer is a renderer.NodeRenderer implementation that
// renders sections.
type SectionMarkdownRenderer struct {
}

// NewSectionMarkdownRenderer returns a new SectionMarkdownRenderer.
func NewSectionMarkdownRenderer() renderer.NodeRenderer {
	r := &SectionMarkdownRenderer{}

	return r
}

// RegisterFuncs implements renderer.NodeRenderer.RegisterFuncs.
func (r *SectionMarkdownRenderer) RegisterFuncs(reg renderer.NodeRendererFuncRegisterer) {
	reg.Register(ast.KindSection, r.renderSection)
}

func (r *SectionMarkdownRenderer) renderSection(
	w util.BufWriter, _ []byte, node gast.Node, entering bool) (gast.WalkStatus, error) {
	n := node.(*ast.Section)
	if entering {
		_, _ = fmt.Fprintf(w, sectionStartFormat, n.Name)
	} else {
		_, _ = fmt.Fprintf(w, sectionEndFormat, n.Name)
	}
